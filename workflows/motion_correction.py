def curate_EPI_space(moco_target):
    ...
    if len(shape) == 4:  # 4D dataset
        ...
        out_file = op.join(tempdir, op.split(moco_target)
                           [-1][:-7] + '_dbl.nii.gz')
...
def create_motion_correction_workflow(name='moco', method='AFNI'):
    ...
    mean_bold = pe.Node(interface=fsl.maths.MeanImage(
        dimension='T'), name='mean_space')
    ...
    motion_correction_workflow.connect(
        input_node, 'which_file_is_EPI_space', EPI_file_selector_node, 'which_file')
    motion_correction_workflow.connect(
        input_node, 'in_files', EPI_file_selector_node, 'in_files')
    ...
    motion_correction_workflow.connect(
        input_node, 'sub_id', datasink, 'container')
    ...
    if method == 'FSL':
        ...
        take_first_TR = pe.MapNode(fsl.ExtractROI(
            t_min=0, t_size=1), name='take_first_TR', iterfield=['in_file'])
        ...
        motion_correction_workflow.connect(
            EPI_file_selector_node, 'raw_EPI_space_file', curate, 'moco_target')
        motion_correction_workflow.connect(
            curate, 'out_file', motion_correct_EPI_space, 'in_file')

        motion_correction_workflow.connect(
            motion_correct_EPI_space, 'out_file', mean_bold, 'in_file')
        motion_correction_workflow.connect(
            mean_bold, 'out_file', motion_correct_all, 'ref_file')

        # the pre-registration
        motion_correction_workflow.connect(
            input_node, 'in_files', take_first_TR, 'in_file')
        motion_correction_workflow.connect(
            take_first_TR, 'roi_file', prereg_flirt_N, 'in_file')
        motion_correction_workflow.connect(
            mean_bold, 'out_file', prereg_flirt_N, 'reference')

        # motion correction across runs
        motion_correction_workflow.connect(
            prereg_flirt_N, 'out_matrix_file', motion_correct_all, 'init')
        motion_correction_workflow.connect(
            input_node, 'in_files', motion_correct_all, 'in_file')

        # output node, for later saving
        motion_correction_workflow.connect(
            mean_bold, 'out_file', output_node, 'EPI_space_file')
        motion_correction_workflow.connect(
            motion_correct_all, 'par_file', output_node, 'motion_correction_parameters')

        motion_correction_workflow.connect(
            motion_correct_all, 'out_file', output_node, 'motion_corrected_files')
        ...
        motion_correction_workflow.connect(
            motion_correct_all, 'par_file', plot_motion, 'in_file')
        motion_correction_workflow.connect(
            plot_motion, 'out_file', output_node, 'motion_correction_plots')

        # and the output

        motion_correction_workflow.connect(
            mean_bold, 'out_file', rename, 'in_file')
        motion_correction_workflow.connect(rename, 'out_file', datasink, 'reg')

        motion_correction_workflow.connect(
            motion_correct_all, 'out_file', datasink, 'mcf')
        motion_correction_workflow.connect(
            motion_correct_all, 'par_file', datasink, 'mcf.motion_pars')
        motion_correction_workflow.connect(
            plot_motion, 'out_file', datasink, 'mcf.motion_plots')
    ...
    if method == 'AFNI':
        ...
        motion_correction_workflow.connect(
            input_node, 'in_files', motion_correct_all, 'in_file')
        motion_correction_workflow.connect(
            mean_bold, 'out_file', motion_correct_all, 'basefile')
        ...
        motion_correction_workflow.connect(
            mean_bold, 'out_file', output_node, 'EPI_space_file')
        motion_correction_workflow.connect(
            motion_correct_all, 'md1d_file', output_node, 'max_displacement_info')
        motion_correction_workflow.connect(
            motion_correct_all, 'oned_file', output_node, 'motion_correction_parameter_info')
        motion_correction_workflow.connect(
            motion_correct_all, 'oned_matrix_save', output_node, 'motion_correction_parameter_matrix')
        ...
        motion_correction_workflow.connect(
            motion_correct_all, 'out_file', datasink, 'mcf')
        motion_correction_workflow.connect(
            motion_correct_all, 'md1d_file', datasink, 'mcf.max_displacement_info')
        motion_correction_workflow.connect(
            motion_correct_all, 'oned_file', datasink, 'mcf.parameter_info')
        motion_correction_workflow.connect(
            motion_correct_all, 'oned_matrix_save', datasink, 'mcf.parameter_matrix')
